...
- Write-design gate auditor that requires first-tranche write designs and allows only the approved Reminders, iCloud Drive, Calendar, Contacts, and Notes apply surfaces.
...
- That only Reminders create/complete/due-date apply, iCloud Drive create-text apply, Calendar create-event apply, Contacts create-contact apply, and Notes create-note apply are currently available, and only after plan approval-token and explicit-confirmation checks.
