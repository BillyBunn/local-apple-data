...
- Write-design gate auditor that requires first-tranche write designs and allows only the approved Reminders and iCloud Drive apply surfaces.
...
- That only Reminders create/complete/due-date apply and iCloud Drive create-text apply are currently available, and only after plan approval-token and explicit-confirmation checks.
