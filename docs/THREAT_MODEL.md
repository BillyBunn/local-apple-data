...
- No mutation of Mail, Notes, Hide My Email, Gmail, iCloud, TCC, launchd, Codex config, or OpenClaw state outside the approved Reminders, iCloud Drive, Calendar, Contacts, Notes, and Mail draft apply gates.
...
- Photos bytes and export paths: Photos exact detail does not return thumbnails, originals, edited assets, videos, or raw file paths. Photos exact export writes one selected local PhotoKit resource to a caller-selected output directory and returns destination metadata only.
...
- Planning confused with mutation: Reminders, iCloud Drive, Calendar, Contacts, Notes, and Mail planning use `plan` naming, read-only MCP annotations, `mutation_applied:false`, `apply_available:true`, deterministic idempotency metadata, and no write calls. Reminders, iCloud Drive, Calendar, Contacts, Notes, and Mail apply use separate non-read-only MCP annotations.
...
Any attachment retrieval, mutation beyond the approved Reminders create/complete/due-date apply surface, iCloud Drive create-text apply surface, Calendar create-event apply surface, Contacts create-contact apply surface, Notes create-note apply surface, and Mail create-draft apply surface, background indexing, arbitrary document extraction, Contact update/delete/note/image retrieval, Notes append/update/delete/move/rich-text mutation, Mail send/reply/forward/archive/move/delete/mark/flag/mailbox-account mutation, generated transcription, broad Messages text search, broad Voice Memos transcript search, broad Reminder content search, authoritative Hide My Email inventory, Hide My Email creation/deactivation/deletion, private iCloud web/API access, browser/keychain credential access, or connector fallback requires a separate explicit design and approval gate.
