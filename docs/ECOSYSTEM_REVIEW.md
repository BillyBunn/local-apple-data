...
Architecture implication: Notes content retrieval is appropriate for a local Mac plugin, but it should remain exact-handle and bounded. Writes need separate preview/apply/read-back gates because other Notes MCP servers expose writes, and that is exactly where privacy and accidental mutation risk rises.
...
- Whether first write support should start with Reminders, Calendar, Notes, Mail drafts, or iCloud Drive text files.
