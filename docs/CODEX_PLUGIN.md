...
Reminders planning is preview-only. `local-apple-data reminders plan` and MCP `reminders_plan_change` return `mutation_applied:false`, `apply_available:false`, idempotency metadata, and an approval fingerprint for a possible future apply gate. They do not call EventKit, read Reminders, or modify Reminders.
...
- It provides Reminders future-change planning only as a non-mutating preview; apply-capable Reminders tools are not exposed.
