...
No first-tranche tool should delete, archive, move, overwrite, bulk edit, or manage account state. The only approved send operation is the bounded Messages send-text gate documented in `docs/V1_24_MESSAGES_SEND_TEXT_WRITE_DESIGN.md`; Mail send and richer Messages send/mutation remain deferred.
...
- Music audio/lyrics/file-path export, play history, ratings/favorites, playlist track dumps, playback/queue control, library import/delete, raw Music database parsing, iCloud media fetch, Music.app automation beyond bounded read-only metadata, or mutation.
- TV video/file-path/artwork/description export, playback or watched state, ratings/favorites, playlist item dumps, playback/queue control, library import/delete, raw TV library parsing, iCloud media fetch, TV.app automation beyond bounded read-only metadata, or mutation.
