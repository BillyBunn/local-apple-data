...
| Reminders | EventKit helper plus legacy SQLite metadata | Title/due metadata | Notes by `reminders:reminder:eventkit:v1:` handle | Not implemented | Reminders permission | EventKit exact handles only for notes; no broad dumps, raw EventKit IDs, or mutation |
