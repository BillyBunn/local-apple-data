...
| Messages | Local Messages `chat.db` plus local Messages attachment files | Chat display-name metadata and selected-chat attachment metadata | Bounded transcript by `messages:chat:v1:` handle, including local `text` and `attributedBody` plaintext fallback; local attachment export by selected `messages:attachment:v1:` handle plus original chat handle | Not implemented | Full Disk Access may be required | No raw attributed-body blobs or attributes, participant identifiers, source media paths, inline bytes, broad attachment export, attachment mutation, reactions, broad text search, send/edit/delete, or Messages.app automation |
