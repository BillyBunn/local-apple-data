...
Use tool filters if a client should remain strictly read-only. The current release exposes four approved write tools: `reminders_apply_change` for Reminders create/complete/due-date apply, `icloud_drive_apply_change` for iCloud Drive create-text apply, `calendar_apply_change` for Calendar create-event apply, and `contacts_apply_change` for Contacts create-contact apply after plan approval-token and explicit-confirmation checks.
