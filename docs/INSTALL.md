...
Use tool filters if a client should remain strictly read-only. The current release exposes one approved write tool, `reminders_apply_change`, for Reminders create/complete/due-date apply after plan approval-token and explicit-confirmation checks.
