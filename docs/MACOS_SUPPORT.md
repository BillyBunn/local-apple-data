...
| Messages | Messages local `chat.db` plus local attachment files for exact selected-chat attachment export | Full Disk Access may be required |
...
- Messages support returns bounded transcripts and can export one exact selected attachment to a caller-selected output directory. It does not return participant identifiers, source media paths, inline attachment bytes, or fetch unavailable iCloud media.
