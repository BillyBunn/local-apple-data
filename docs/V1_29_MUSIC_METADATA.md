...
Add a read-only Apple Music surface for locally synced Music library track and playlist metadata without returning audio bytes, lyrics, file paths, raw identifiers, play history, ratings, playlist track dumps, or mutating Music.app.
...
Track search returns bounded metadata for specific title, artist, album, album-artist, or genre queries. Playlist search returns bounded playlist name/type/count metadata for specific playlist-name queries.
...
- Redaction booleans confirming raw identifiers, file paths, audio, lyrics, play history, ratings, and playlist track lists are not returned.
...
- Playlist track dumps, playlist creation/update/delete, library import/delete, playback/queue control, Music.app mutation, iCloud media fetch, or Apple Music network/API access.
...
- Synthetic adapter tests cover search, exact track get, exact playlist get, invalid handles, broad-query rejection, automation errors, and raw identifier/path/audio/lyrics redaction.
- CLI tests cover all four Music commands.
